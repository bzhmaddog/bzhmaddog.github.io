...
class CanvasLayer extends BaseLayer {
    ...
    drawImage(img, _options) {
        ...
        var options = this._buildOptions(new Options(_options), defaultOptions);
        this._contentBuffer.context.drawImage(img, options.get('left'), options.get('top'), options.get('width'), options.get('height'));
        this._layerUpdated();
    }
    _buildOptions(_options, _defaultOptions) {
        var options = Object.assign(this._options, _defaultOptions, _options);
